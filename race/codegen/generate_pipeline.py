...
class GeneratePipeline():
    def __init__(
        ...
        assert backend in ["vllm", "hf", "openai", "genai", "azure", "together", "cohere"], f"Invalid backend {backend}"
    ...
    def pipeline_simple(self):
        ...
        final_samples = []
        for s in samples:
            for sol in s['solution']:
                final_samples.append(dict(task_id=s['task_id'], solution=sol))
        
        file_name = os.path.join(self.root, f'{self.dataset_name}_{self.dim}_{self.model_name}.jsonl')
        write_jsonl(file_name, final_samples)
