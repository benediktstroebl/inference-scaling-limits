...
class OpenAIChatDecoder(DecoderBase):
    def __init__(self, name: str, backend: str, base_url=None, api_key=None, **kwargs) -> None:
        super().__init__(name, **kwargs)
        ...
        if backend == "openai":
            ...
            self.client = openai.OpenAI(base_url="https://api.together.xyz/v1", api_key=os.environ.get("TOGETHER_API_KEY"))
    ...
    def codegen(
        ...
        **kwargs
        ...
        if "mistral" not in self.name.lower():
            if len(outputs) == 1:
                return outputs[0]
            else:
                return outputs
...
def make_model(
    ...
    elif backend in ["openai", "azure", "together"]:
